...
fun HomeScreen(
    ...
    Column(
        ...
        DividendTabRow(
            selected = state.selectedTab.toDividendTab(),
            onSelect = { onEvent(HomeEvents.ClickedTab(it.toHomeDividendTab())) },
        )
        ...
        HoldingDividendList(
            ...
            selectedTab = state.selectedTab.toDividendTab(),
            displayCurrency = state.displayCurrency,
            usdKrw = state.usdKrw,
            modifier = Modifier
                .fillMaxWidth()
...
/** Map commonMain HomeDividendTab to designsystem DividendTab. */
private fun HomeDividendTab.toDividendTab(): DividendTab = when (this) {
    HomeDividendTab.YEARLY -> DividendTab.YEARLY
    HomeDividendTab.QUARTERLY -> DividendTab.QUARTERLY
    HomeDividendTab.MONTHLY -> DividendTab.MONTHLY
}

/** Map designsystem DividendTab back to commonMain HomeDividendTab. */
private fun DividendTab.toHomeDividendTab(): HomeDividendTab = when (this) {
    DividendTab.YEARLY -> HomeDividendTab.YEARLY
    DividendTab.QUARTERLY -> HomeDividendTab.QUARTERLY
    DividendTab.MONTHLY -> HomeDividendTab.MONTHLY
}
