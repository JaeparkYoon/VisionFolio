...
interface SnapshotDao {
    ...
    @Query("SELECT * FROM portfolio_snapshots WHERE timestamp < :before ORDER BY timestamp DESC LIMIT 1")
    fun observeLatestBefore(before: Long): Flow<SnapshotEntity?>

    @Query("SELECT MIN(timestamp) FROM portfolio_snapshots")
    fun observeOldestTimestamp(): Flow<Long?>

    @Query("SELECT MAX(timestamp) FROM portfolio_snapshots")
